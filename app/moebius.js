...
fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P start: ${Date.now()} pid:${process.pid}\n`);
...
fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P before-lock: ${Date.now()} pid:${process.pid}\n`);
if (!electron.app.requestSingleInstanceLock({files: _initial_files})) {
    fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P2 exiting: ${Date.now()} pid:${process.pid}\n`);
    electron.app.exit(0);
}
fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P1 lock-acquired: ${Date.now()} pid:${process.pid}\n`);
...
async function open_file(file) {
    ...
    fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P1 open_file start: ${Date.now()}\n`);
    const win = await new_document_window();
    fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P1 window ready: ${Date.now()}\n`);
...
electron.app.on("second-instance", (event, argv_new, working_dir, additionalData) => {
    fs.appendFileSync("C:\\Users\\Public\\moebius_timing.txt", `P1 second-instance fired: ${Date.now()}\n`);
