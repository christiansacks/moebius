...
function draw_half_block_ellipse(sx, sy, dx, dy, col, col_rgb = undefined, col_idx = undefined) {
    const coords = ellipse_coords(sx, sy, dx, dy);
    if (!coords) return;
    for (let i = 0; i < coords.length; i += 2) {
        brushes.single_half_block_line(coords[i].x - 1, coords[i].y, coords[i + 1].x, coords[i + 1].y, col, false, col_rgb, col_idx);
    }
}
...
function draw_custom_block_ellipse(sx, sy, dx, dy, fg, bg, colors_ext = {}) {
    const coords = ellipse_coords(sx, sy, dx, dy);
    if (!coords) return;
    for (let i = 0; i < coords.length; i += 2) {
        brushes.single_custom_block_line(coords[i].x - 1, coords[i].y, coords[i + 1].x, coords[i + 1].y, fg, bg, false, colors_ext);
    }
}
...
function draw_shaded_block_ellipse(sx, sy, dx, dy, fg, bg, reduce, colors_ext = {}) {
    ...
    if (!coords) return;
    for (let i = 0; i < coords.length; i += 2) {
        brushes.single_shading_block_line(coords[i].x - 1, coords[i].y, coords[i + 1].x, coords[i + 1].y, fg, bg, reduce, false, colors_ext);
    }
}
...
function draw_replace_color_block_ellipse(sx, sy, dx, dy, to, from, to_rgb, to_idx, from_rgb, from_idx) {
    ...
    if (!coords) return;
    for (let i = 0; i < coords.length; i += 2) {
        brushes.single_replace_color_line(coords[i].x - 1, coords[i].y, coords[i + 1].x, coords[i + 1].y, to, from, false, to_rgb, to_idx, from_rgb, from_idx);
    }
}
...
mouse.on("up", (x, y, half_y, button) => {
    ...
    if (toolbar.mode == toolbar.modes.HALF_BLOCK) {
        if (clear) {
            ...
            draw_half_block_ellipse(mouse.start.x, mouse.start.half_y, x, half_y, use_fg ? fg : bg, use_fg ? fg_rgb : bg_rgb, use_fg ? fg_idx : bg_idx);
        }
        return;
    }
    if (clear) {
        ...
        switch (toolbar.mode) {
            case toolbar.modes.CUSTOM_BLOCK:
                draw_custom_block_ellipse(mouse.start.x, mouse.start.y, x, y, fg, bg, colors_ext);
                break;
